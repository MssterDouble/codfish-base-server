...
public class HandelerHomework {
	public static void run(String requestBody, HttpServletResponse response, String apiName) {
...
//		ClassObj classObj = gsinput.fromJson(requestBody, ClassObj.class);
		// 获取班级是否重复注册
		List classList = getClassList("schoolName", CFMPRGCL.getSchoolName(), CFMPRGCL.getClassName(),
				CFMPRGCL.getTeacherId(), "");
	...
	static List getClassList(String type, String schoolName, String className, String teacherId, String classId) {
		...
		if (type == "schoolName") { // 根据校+班级名称查找状态正常的班级
			querySql = "SELECT * FROM t_hw_class_inf WHERE schoolName='" + classObj.getSchoolName() + "'"
					+ " AND className='" + classObj.getClassName() + "'" + " AND '" + timeMaker("currentDate")
					+ "' >= startTime" + " AND '" + timeMaker("currentDate") + "' <= endTime" + " AND stt='1';";
		} else if (type == "classId") { // 根据班级ID查找班级信息
			querySql = "SELECT * FROM t_hw_class_inf WHERE classId='" + classId + "'" + " AND '"
					+ timeMaker("currentDate") + "' >= startTime" + " AND '" + timeMaker("currentDate") + "' <= endTime"
					+ " AND stt='1';";
